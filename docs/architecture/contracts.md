...
| `GET /workspaces/{workspace}/{section?}` | A workspace's pages (`section`: `modules`, `notes`, `calendar`, `progress`; none for Overview). Another student's workspace answers 404 like a missing one. `/` is My workspaces for a student |
