...
| `GET /workspaces/{workspace}/notes/{note}` | A note and its editor (`resources/js/note/`), which saves through `PUT /api/v1/notes/{id}`. The note must belong to that workspace; anything else answers 404. Trash and restore: `App\Livewire\Workspaces\NoteActions` |
...
| `POST /api/v1/notes`, `GET /api/v1/sync/tombstones` | Draft | M2 step 3b | Creating a note from the browser with its own ID, and deletion records for drafts (ADR 0003 §5.4) |
