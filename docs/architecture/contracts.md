...
| `Study\Folders` | `tree(Principal, workspaceId): list<FolderDetails>` (parents before children, siblings in order) · `find(Principal, id)` · `create(Principal, 'module'\|'folder', parentId, name)` · `rename(Principal, id, name)` · `move(Principal, id, 'module'\|'folder', parentId)` · `reorder(Principal, id, position)` · `delete(Principal, id)`; constants `MAX_DEPTH` (8), `MAX_NAME` | As `Study\Workspaces`, and only within one workspace. `409 too_deep` past 8 levels, `409 invalid_move` into itself, `409 not_empty` for a folder with folders inside. Not journalled |
...
| `GET /workspaces/{workspace}/{section?}` | A workspace's pages (`section`: `modules`, `notes`, `calendar`, `progress`; none for Overview). Another student's workspace answers 404 like a missing one. `/` is My workspaces for a student. The Modules section is `App\Livewire\Workspaces\Modules` (locked workspace and target IDs) |
